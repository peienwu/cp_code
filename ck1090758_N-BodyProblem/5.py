...
r1 = m2 / (m1+m2)*d
r2 = m1 / (m1+m2)*d
v_m1 = m2 * (G/(d*(m1+m2)))**0.5
v_m2 = m1 * (G/(d*(m1+m2)))**0.5

alpha = m2 / (m1+m2)

scene = canvas(align = 'left',width=700, height=400, center=vec(0,0,0),background=color.white)

ball_m1 = sphere(pos=vector(-r1,0,0), radius=0.2, color = color.yellow, make_trail=True)
ball_m2 = sphere(pos=vector(r2,0,0), radius=0.1, color = color.red, make_trail=True)

ball_mass = sphere(pos=((m1*ball_m1.pos)+(m2*ball_m2.pos))/(m1+m2), radius=0.05, color = color.black, make_trail=True)
ball_m1_v = vec(0,-v_m1/2,0)
ball_m2_v = vec(0,v_m2/2,0)

gd = graph(align = 'right',width=400,height=400,title='K1+K2+U=E', xtitle='t', ytitle='E,U,K1,K2',foreground=color.black,background=color.white)
K1_curve = gcurve(color=color.black)
